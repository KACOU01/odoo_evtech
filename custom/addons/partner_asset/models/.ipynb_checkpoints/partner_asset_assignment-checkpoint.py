...
class PartnerAssetAssignment(models.Model):
    ...
    asset_id = fields.Many2one('partner.asset', string="Congélateur", domain=[('is_assign', '=', False), ('active', '=', True)], tracking=True)
    partner_id = fields.Many2one('res.partner', string='Client', domain=[('is_customer', '=', True)], tracking=True)
